...
- (NSURLSessionDataTask *)_sendPostRequestWithParameters:(UPPParameters *)parameters action:(NSString *)action success:(UPPSuccessBlock)successBlock
{
    ...
    return [self.sessionManager POST:[self.controlURL absoluteString] parameters:wrapped headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (successBlock) {
...
- (NSURLSessionDataTask *)_sendPostRequestWithParameters:(UPPParameters *)parameters action:(NSString *)action completion:(UPPResponseBlock)completion
{
    ...
    return [self.sessionManager POST:[self.controlURL absoluteString] parameters:wrapped headers:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
